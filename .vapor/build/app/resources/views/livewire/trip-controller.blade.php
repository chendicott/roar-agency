            ...
            <div class="max-w-6xl">
                <div class="flex border-b border-gray-200 pb-6 flex justify-between items-center">
                    <div class="flex flex-col">
                        ...
                        <h3 class="text-lg font-medium text-gray-500 leading-5">{{$trip->trip_start_date}} - {{$trip->trip_end_date}}</h3>
                    </div>
                    <div class="text-medium font-bold uppercase text-gray-500 leading-5">
                    ...
                    <div x-show="tab === 'Details'">
                            ...
                            <div class="flex">
                                ...
                                @foreach ($trip->participants as $participant)
                                    <div class="flex justify-between items-center w-full">
                                        <div class="flex flex-col">
                                            <div class="text-base text-gray-900">{{$participant->first_name}} {{$participant->last_name}}</div>
                                            <div class="text-sm text-gray-500">{{$participant->email}}</div>
                                        </div>
                                        @if (auth()->user()->isAdmin())
                                            <a href="/admin/profile/{{$participant->id}}" class="block text-sm text-sky-500">View Profile</a>
                                        @else
                                            <a href="/profile/{{$participant->id}}" class="block text-sm text-sky-500">View Profile</a>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="py-6 border-b border-gray-200 px-5">
                            <div class="flex">
                                ...
                                @foreach ($trip->supportWorkers as $supportWorker)
                                    <div class="flex justify-between items-center w-full">
                                        <div class="flex flex-col">
                                            <div class="text-base text-gray-900">{{$supportWorker->first_name}} {{$supportWorker->last_name}}</div>
                                            <div class="text-sm text-gray-500">{{$supportWorker->email}}</div>
                                        </div>
                                        @if (auth()->user()->isAdmin())
                                            <a href="/admin/profile/{{$supportWorker->id}}" class="block text-sm text-sky-500">View Profile</a>
                                        @else
                                            <a href="/profile/{{$supportWorker->id}}" class="block text-sm text-sky-500">View Profile</a>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="flex items-center w-full">
                                    <div class="flex flex-col">
                                        ...
                                        <div class="text-base text-gray-900">{{$trip->trip_start_date}}</div>
                                    </div>
                                    <div class="flex flex-col pl-10">
                                        <x-label>End Date</x-label>
                                        <div class="text-base text-gray-900">{{$trip->trip_end_date}}</div>
                                    </div>
                                </div>
                        ...
                        <div class="py-6 border-b border-gray-200 px-5">
                            <div class="flex">
                                ...
                                <div class="flex items-center w-full">
                                    <div class="text-base text-gray-900">{{$trip->trip_location}}</div>
                                </div>
                        ...
                        @foreach ($trip->trip_data as $key => $value)
                           ...
                           @if ($key != "supportWorker" && $key != "emotionalSupport" && $key != "quote" && $key != "startDate" && $key != "endDate" && $key != "location")
                                <div class="py-6 border-b border-gray-200 px-5">
                                    <div class="flex">
                                        <div class="flex flex-col w-60">
                                            <div class="text-sm text-gray-500">{{$this->camelCaseToFriendlyName($key)}}</div>
                                        </div>
...
{{--                    </div>--}}
                    ...
                    <div x-show="tab === 'Budget'">
                        ...
                        @if (isset($trip->trip_data['quote']))
                            <div class="py-6 border-b border-gray-200 px-5">
                                <div class="flex">
                                    ...
                                    <div class="flex items-center w-full">
                                        <div class="text-base text-gray-900">{{$trip->trip_data['quote']['numberOfNights']}}</div>
                                    </div>
                            ...
                            <div class="py-6 border-b border-gray-200 px-5">
                                <div class="flex">
                                    ...
                                    <div class="flex items-center w-full">
                                        <div class="text-base text-gray-900">{{$trip->trip_data['quote']['supportWorkerPreference']}}</div>
                                    </div>
                            ...
                            <div class="py-6 border-b border-gray-200 px-5">
                                <div class="flex">
                                    ...
                                    <div class="flex items-center w-full">
                                        <div class="text-base text-gray-900">{{$trip->trip_data['quote']['receiveSupportFromRoar']}}</div>
                                    </div>
                        ...
                        @if (auth()->user()->isAdmin())
                            ...
                            <div class="text-gray-500">Budget is only editable by Roar Staff, but is viewable by all users</div>
                            <textarea wire:model="trip.budget" class="mt-4 w-full h-64 border border-gray-300 rounded-md"></textarea>
                            <div class="flex justify-end mt-5">
                                <x-button wire:click="updateBudget">Update Budget</x-button>
                            </div>
                        @else
                            @if ($trip->budget === "")
                                <div class="bg-gray-50 mt-10 p-3 text-center text-gray-900">Budget to be updated by STA staff, check back soon</div>
                            @else
                                <div class="bg-gray-50 mt-10 p-3 text-center text-gray-900">Budget updated by STA staff</div>
                                {{$trip->budget}}
                            @endif
                        @endif
                    ...
                    <div x-show="tab === 'Incidents'">
                        ...
                        @if ($trip->incidentReports->count() === 0)
                            <div class="bg-gray-50 mt-10 p-3 text-center text-gray-900">No incident reports have been submitted</div>
                        @else
                            @foreach ($trip->incidentReports as $incident)
                                <div class="py-6 border-b border-gray-200 px-5">
                                    <div class="flex-col divide-gray-200">
                                            <div class="text-sm text-gray-900">{{$incident->incident_data['incident']}}</div>
                                            <div class="text-sm text-gray-500 mt-3"><b>Date:</b> {{$incident->incident_data['date']}}</div>
                                            <div class="text-sm text-gray-500"><b>Participant:</b> {{$incident->participant->first_name}} {{$incident->participant->last_name}}</div>
                                            <div class="text-sm text-gray-500"><b>Submitted By:</b> {{$incident->submittedBy->first_name}} {{$incident->submittedBy->last_name}}</div>
                                    </div>
                                </div>
                            @endforeach
                        @endif
                        ...
                        <div class="mt-10 border border-gray-200 rounded p-4">
                            <h3 class="text-lg font-medium text-gray-900 leading-5">Submit Incident Report</h3>
                            <x-label class="mt-4">Select participant that this report relates to</x-label>
                            <select wire:model="incident_participant" class="w-full border border-gray-300 rounded-md">
                            ...
                            <input wire:model="incident_date" type="date" class="w-full border border-gray-300 rounded-md">
                            <x-input-error for="incident_date" class="mt-2" />

                            <div class="mt-1">
                                <x-label class="mt-4">Incident Details</x-label>
                                <textarea wire:model="incident" class="w-full border border-gray-300 rounded-md"></textarea>
